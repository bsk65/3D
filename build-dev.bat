...
REM Opret 3D-dev mappe hvis den ikke findes
if not exist "3D-dev" mkdir "3D-dev"

REM Kopier dist-dev indhold til 3D-dev/
xcopy dist-dev\* 3D-dev\ /E /Y /Q

REM Git: tilfoej kun testmappen
git add 3D-dev/
git diff --cached --quiet
if errorlevel 1 (
  git commit -m "Test-build opdatering [3D-dev]"
  git push origin dev
  echo === Faerdig - test-appen er live om 1-2 min pa /3D-dev/ ===
