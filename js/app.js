...
document.addEventListener('DOMContentLoaded', () => {
  // Auth state listener — Firebase compat
  auth.onAuthStateChanged(async user => {
    if (user) {
      ...
      try {
        ...
        if (snap.exists) {
          const d = snap.data();
          // Støtter både gamle feltnavne (yam/e-mail) og nye (name/email)
          state.profile = {
...
async function onLogin() {
  ...
  await Promise.all([ loadUserFriends(), loadUserRounds(), loadCoursesData() ]);
...
function switchTab(tab) {
  ...
  if (tab === 'friends') { loadUserFriends(); renderAdminSection(); }
...
async function loadUserFriends() {
  state.friends = await loadFriends(state.user.uid);
  renderFriendsList();
  renderQuickFriends();
}
...
async function loadUserRounds() {
  try {
    const snap = await db.collection('brugere').doc(state.user.uid)
      .collection('runder').get();
    state.rounds = snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .sort((a, b) => {
        const ta = a.created?.toMillis ? a.created.toMillis() : (a.skabt?.toMillis ? a.skabt.toMillis() : 0);
        const tb = b.created?.toMillis ? b.created.toMillis() : (b.skabt?.toMillis ? b.skabt.toMillis() : 0);
        return tb - ta;
      });
    renderRoundsList();
  } catch(err) { console.error('loadUserRounds:', err); }
}
...
async function tryResumeRound() {
  try {
    ...
    const age = Date.now() - (data.created?.toMillis ? data.created.toMillis() : Date.now());
    ...
    if (confirm('Genoptag den igangværende runde?')) {
      ...
      state.round.traversalOrder = data.traversalOrder;
      state.round.traversalPos   = data.traversalPos || 0;
      if (state.round.courseId) {
        state.course = await loadCourse(state.round.courseId);
      }
      showActivePanel();
      renderShooters();
      updateTopBar();
...
function populateCourseDropdown() {
  ...
  state.courses.forEach(c => {
    ...
    o.value = c.id; o.textContent = `${c.name} (${c.numTargets} mål)`;
    sel.appendChild(o);
  });
  // Update start target when course changes
  sel.onchange = () => {
...
function addParticipant(id, name, isGuest) {
  ...
  div.innerHTML = `<span class="pchip-name">🎯 ${name}</span>
    <button class="pchip-rm" onclick="document.getElementById('chip-${id}').remove()">✕</button>`;
...
async function startRound() {
  ...
  state.round = {
    ...
    created: Date.now(), traversalOrder: buildOrder(startIdx, numTargets), traversalPos: 0,
  };
  ...
  // Vis aktiv skydning med det samme - gem i baggrunden
  showActivePanel();
  renderShooters();
  updateTopBar();
  saveActiveRound(); // gem asynkront uden at blokere UI
}
...
function updateTopBar() {
  ...
  const allVals   = state.round.shooters.flatMap(s => s.scores.flat().filter(v => v !== null && v !== undefined));
...
function skipToTarget() {
  document.getElementById('skip-input').max = state.round?.numTargets || 30;
...
async function finishRound() {
  ...
  try {
    const ref = await db.collection('brugere').doc(state.user.uid)
      .collection('runder').add({ ...roundData, created: firebase.firestore.FieldValue.serverTimestamp() });

    if (state.round.courseId) {
      const winner = findWinner(state.round.shooters);
      await addCourseVisit(state.round.courseId, {
        roundId: ref.id, date: new Date().toLocaleDateString('da-DK'),
...
async function abortRound() {
  ...
  releaseWakeLock();
  try { await db.collection('brugere').doc(state.user.uid).collection('aktiv').doc('runde').delete(); } catch(e){}
  state.round = null;
  showSetupPanel();
}
...
function buildDistribution(round) {
  return '<div class="dist-grid">' + round.shooters.map(s => {
    ...
    return `<div class="dist-card"><div class="dist-name">${s.name}</div>
      ${Object.entries(d).map(([k,v]) => `<div class="dist-row"><span>${k}</span><span>${v}x</span></div>`).join('')}
    </div>`;
  }).join('') + '</div>';
}
  ...
  state.rounds.forEach((r, i) => {
    ...
    card.querySelector('.del-btn').onclick = async e => {
      ...
      if (!state.deleteConfirm[key]) {
        ...
        await db.collection('brugere').doc(state.user.uid).collection('runder').doc(r.id).delete();
        await loadUserRounds();
      }
    };
    el.appendChild(card);
...
function showRoundPopup(round) {
  ...
  document.getElementById('rpop-body').innerHTML = `<h3 style="font-family:var(--font-display);color:var(--accent);margin-bottom:12px;">${round.name}</h3>` + buildResultsTable(round) + buildDistribution(round);
}
...
function initCourseMap(course) {
  ...
  L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
    { attribution: 'Esri', maxZoom: 19 }).addTo(state.courseMap);
  const bounds = [];
  (course.targets||[]).forEach((t, i) => {
    ...
    })}).addTo(state.courseMap).bindPopup(`<b>${i+1}. ${t.name||'Mål'}</b>${t.imageUrl?`<br><img src="${t.imageUrl}" style="max-width:120px;border-radius:4px;margin-top:4px;"/>`:''}`)
  });
...
function renderVisits(course) {
  ...
  visits.forEach((v, idx) => {
    ...
    card.innerHTML = `
      ...
      <div style="font-size:12px;color:var(--text-muted);">${(v.participants||[]).join(', ')}</div>
      ${v.winner ? `<div style="font-size:13px;color:var(--accent);font-weight:600;">🏆 ${v.winner} (${v.winnerScore} pt)</div>` : ''}`;
    el.appendChild(card);
  });
}
...
function showRouteOnMap(points) {
  ...
  state.courseMapLayer = L.polyline(points.map(p => [p.lat, p.lng]), { color: '#e8a020', weight: 3, dashArray: '8,4' }).addTo(state.courseMap);
...
async function saveCourseEdit() {
  ...
  if (!name) return;
  await updateCourse(state.currentCourse.id, { name, location: loc });
...
function renderFriendsList() {
  ...
  state.friends.forEach(f => {
    ...
    card.innerHTML = `
      ...
      <div class="factions">
        <button class="btn-icon" onclick="openFriendModal(${JSON.stringify(f).replace(/"/g,'&quot;')})">✏️</button>
        <button class="btn-icon" style="color:var(--danger);" onclick="doDeleteFriend('${f.id}','${f.name.replace(/'/g,"\\'")}')">🗑</button>
      </div>`;
    el.appendChild(card);
  });
}
...
async function saveFriendModal() {
  ...
  if (!data.name) return;
  await saveFriend(state.user.uid, data, state.editFriendId);
...
async function doDeleteFriend(id, name) {
  ...
  await deleteFriend(state.user.uid, id);
  await loadUserFriends();
}
...
async function renderAdminSection() {
  ...
  users.forEach(u => {
    ...
    row.innerHTML = `<span class="un">${u.name||'—'}</span><span class="ue">${u.email}</span><span class="ud">${date}</span>`;
    el.appendChild(row);
  });
}
...
function showQR() {
  ...
  if (typeof QRCode !== 'undefined') {
    new QRCode(el, { text: window.location.href, width: 200, height: 200, colorDark: '#1a3a1a', colorLight: '#fff' });
  }
}
