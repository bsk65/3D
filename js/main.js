// js/main.js — Indgangspunkt for Vite bundle
import './firebase-instance.js'
import { showAuthTab, doLogin, doSignup, doForgot, doLogout, initAuthListener } from './auth.js'
import { subscribeCourses, loadCourse, createCourse, updateCourse, updateTarget,
         uploadTargetImage, addCourseVisit, removeCourseVisit, deleteCourse, compressImage } from './courses.js'
import { loadLocalData, saveLocalData, saveFriendLocal, deleteFriendLocal,
         checkIsAdmin, loadAllUsers, findUserByEmail, addAdmin } from './friends.js'
import { parseRoute, serializeRoute, startTracking, stopTracking,
         toggleGpsPause, getCurrentPosition, findNearestTarget,
         formatDuration, formatDistance } from './gps.js'
import { SCORE_VALUES, makeShooter, normalizeScores, serializeRound, deserializeRound,
         calcTotal, calcAverage, calcTargetAverage, calcDistribution,
         findWinner, isBelowThreshold, countScored, scoreVal, parseScores } from './scoring.js'
import { db, auth } from './firebase-instance.js'
import { collection, doc, setDoc, deleteDoc, serverTimestamp } from 'firebase/firestore'

// Eksponer funktioner globalt så HTML onclick virker
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/3D/sw.js').catch(e => console.warn('SW:', e))
  }
...
// ─── LOGIN/LOGOUT ─────────────────────────────────────────────────────────────
async function onLogin(user, profile) {
  state.user = user
  ...
  // Subscribe til baner fra Firestore (bruger cache ved første load)
  if (state.unsubCourses) state.unsubCourses()
  state.unsubCourses = subscribeCourses(courses => {
...
window.addParticipant = function(id, name, isGuest) {
...
function renderQuickFriends() {
  ...
  state.friends.forEach(f => {
    ...
    btn.onclick = () => window.addParticipant(f.id, f.name)
    el.appendChild(btn)
  })
}
...
function updateTopBar() {
  ...
  document.getElementById('edit-target-btn').classList.toggle('hidden',
    !(state.isAdmin && state.round.courseId))
  document.getElementById('next-btn').textContent =
    state.round.traversalPos === n - 1 ? 'AFSLUT →' : 'NÆSTE →'

  const tAvg = calcTargetAverage(state.round.shooters, tIdx)
  document.getElementById('target-avg').textContent =
    tAvg !== null ? `Gns. dette mål: ${tAvg}` : ''
}
    ...
    card.innerHTML = `
      <div class="sh-head">
        ...
        <div class="sh-mini"><div class="sh-mini-lbl">RUNDE</div>
          <div class="sh-mini-val">${total}</div></div>
      </div>
...
window.setScore = function(shooterIdx, targetIdx, arrowIdx, val) {
...
async function saveActiveRound() {
  ...
  try {
    await setDoc(doc(db, 'brugere', state.user.uid, 'aktiv', 'runde'),
...
async function tryResumeRound() {
  try {
    ...
    if (age > 24 * 60 * 60 * 1000) {
      await deleteDoc(doc(db, 'brugere', state.user.uid, 'aktiv', 'runde'))
      return
    }
...
window.finishRound = async function() {
  ...
  // Gem lokalt øjeblikkeligt
  state.rounds.unshift({ ...roundData, created: { toDate: () => new Date(), toMillis: () => Date.now() } })
  saveLocalData(state.friends, state.rounds)
  renderRoundsList()

  // Backup til Firestore i baggrunden
  if (state.round.courseId) {
    ...
    addCourseVisit(state.round.courseId, {
      ...
      gpsRoute: gpsData.route || null, gpsDuration: gpsData.duration || null,
      gpsDistance: gpsData.distance || null,
    }).catch(console.error)
  }
  deleteDoc(doc(db, 'brugere', state.user.uid, 'aktiv', 'runde')).catch(console.error)
...
function renderRoundsList() {
  ...
  state.rounds.forEach(r => {
    ...
    const date     = r.created?.toDate ? r.created.toDate().toLocaleDateString('da-DK') :
                     (r.created ? new Date(r.created).toLocaleDateString('da-DK') : '—')
    const card     = document.createElement('div')
...
function initCourseMap(course) {
  ...
  ;(course.targets||[]).forEach((t, i) => {
    ...
    window.L.marker([t.gps.lat, t.gps.lng], { icon: window.L.divIcon({
      className: '',
      ...
      .bindPopup(`<b>${i+1}. ${t.name||'Mål'}</b>
        ${t.emoji ? `<br>${t.emoji}` : ''}
        ${t.imageUrl ? `<br><img src="${t.imageUrl}" style="max-width:140px;border-radius:4px;" />` : ''}`)
  })
...
window.showRouteOnMap = function(points) {
  ...
  state.courseMapLayer = window.L.polyline(points.map(p => [p.lat,p.lng]),
...
window.toggleMyPos = async function() {
  ...
  if (sw.classList.contains('on')) {
    try {
      ...
      window.L.circle([pos.lat,pos.lng], { radius:10, color:'#2a7ae8', fillOpacity:0.7 })
        .addTo(state.courseMap)
...
window.saveFriendModal = function() {
  ...
  if (!data.name) return
  state.friends = saveFriendLocal(state.friends, data, state.editFriendId)
  saveLocalData(state.friends, state.rounds)
...
window.doDeleteFriend = function(id, name) {
  ...
  state.friends = deleteFriendLocal(state.friends, id)
  saveLocalData(state.friends, state.rounds)
  renderFriendsList()
  renderQuickFriends()
}
...
window.addGuest = function() {
  ...
  if (!name) return
  window.addParticipant(`guest-${Date.now()}`, name, true)
